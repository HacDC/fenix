...
async fn main(spawner: Spawner) -> ! {
    ...
    let interface_variant =
        iv::GenericSx126xInterfaceVariant::new(rst, dio1, busy, None, None).unwrap();
    let mut lora = LoRa::new(
        sx126x::Sx126x::new(spi_device, interface_variant, lora_config::RADIO_CONFIG),
        false,
        Delay,
    )
    .await
    ...
    match lora
        .prepare_for_rx(RxMode::Continuous, &modulation_config, &rx_packet_config)
        .await
    {
        Ok(_) => {
